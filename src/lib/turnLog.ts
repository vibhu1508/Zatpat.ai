...
export function logAnswer(text: string, t: Trace) {
  ...
  console.log(
    `${t.totalMs} ms  ·  ${t.queryClass} → ${strategy}  ·  k=${t.k}  ·  ${t.tokensIn}→${t.tokensOut} tok`,
  );
  console.log(t.routeReason);
  console.table(
    t.stages.map((s) => ({ stage: s.label, ms: s.ms ?? null })),
  );
  console.table(
    t.chunks.map((c, i) => ({ '#': i + 1, source: c.source, span: c.span, score: c.score })),
  );
  if (fired.length) {
    console.log(
