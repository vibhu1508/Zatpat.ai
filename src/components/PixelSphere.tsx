...
const PALETTE = {
  idle: { core: new THREE.Color('#c3d2e6'), edge: new THREE.Color('#33455f') },
...
export default function PixelSphere({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const bloom = new UnrealBloomPass(new THREE.Vector2(1, 1), 0.45, 0.5, 0.26);
    ...
    // Spring state for the shell opening/closing.
    let spread = 0;
    let spreadV = 0;
    let raf = 0;
    ...
    const tick = () => {
      ...
      const active = field.speaker !== 'idle';
      if (!active) field.decay(0.90);
      ...
      bloom.strength = 0.42 + field.level * 0.40 + field.burst * 0.45;

      // A slow drift so the cloud never presents the same silhouette twice.
      cloud.rotation.y += dt * (0.055 + field.level * 0.10);
      cloud.rotation.x = Math.sin(t * 0.16) * 0.14;
