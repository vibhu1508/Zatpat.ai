...
const ask = (q: string, history: string[] = []) =>
  eng.answer({ query: q, history, guardrails: DEFAULT_GUARDRAILS, languageCode: 'en-IN' });
...
for (const e of entries.slice(0, 200)) {
  for (const f of fu) {
    const r = ask(f, [e.engQuery]);
    if (r.trace.grounded && r.english === e.engAnswer) stayed++;
  }
}
